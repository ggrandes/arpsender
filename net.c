...
void arp_test(void)
{
	...
	//char src_mac[6] = {0x80, 0x60, 0x97, 0x75, 0xA4, 0xA4};
	//char src_mac[6] = {0x00, 0x04, 0x76, 0xe2, 0x5b, 0xd2};
	//char dst_mac[6] = {0x00, 0x60, 0x97, 0x72, 0x4E, 0xB5};
	//char dst_mac[6] = {0x00,0x08,0x02,0xfb,0x91,0xa3};
	char dst_mac[6] = {0xff,0xff,0xff,0xff,0xff,0xff};
	char sender[6] = {0x66, 0x1A, 0xDE, 0xAD, 0xBE, 0xEF};
	//char sender[6] = {0x00, 0x04, 0x76, 0xe2, 0x5b, 0xd2};
	
printf("creando sock\n");
	linksock = tap(eth_device, 1);
printf("creado sock %d\n", linksock);
	as.src_mac = sender;
	as.dst_mac = dst_mac;
	as.oper = htons(ARPOP_REPLY);	/* ARPOP_REQUEST */
	as.oper = htons(ARPOP_REQUEST);
	as.sender_mac = sender;
	as.sender_addr = inet_addr("9.9.9.9");
	as.target_mac = dst_mac;
	as.target_addr = inet_addr("10.1.2.42");
printf("construido %d\n", linksock);
